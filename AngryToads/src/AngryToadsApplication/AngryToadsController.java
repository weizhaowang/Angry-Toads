...
<<<<<<< HEAD
	...
	AngryToadsController(AngryToadsArea m, AngryToadsPanel v) {
		this.m_stage = m;
		m_view = v;
		drawer = v.getSDDraw();
	...
	public void postSolve(Contact contact, ContactImpulse impulse) {
		...
		if (contact.m_fixtureA.m_filter.groupIndex == -1 || contact.m_fixtureB.m_filter.groupIndex == -1) {
			...
			for (int i = 0; i < contact.getManifold().pointCount; i++) {
				...
				if (impulse.normalImpulses[i] > 0.8) {
					...
					/*
					 * try { //music.birdScream(); } catch (IOException ex) { }
					 */
				}
			}
		}
		if (contact.m_fixtureA.m_filter.groupIndex == 1 || contact.m_fixtureB.m_filter.groupIndex == 1) {
			...
			for (int i = 0; i < contact.getManifold().pointCount; i++) {
				// System.out.print("wood impulse :
				// "+impulse.normalImpulses[i]+"\n");
				if (impulse.normalImpulses[i] > 3.1f) {
					// System.out.print("wood impulse :
					// "+impulse.normalImpulses[i]+"\n");
					/*
					 * try { //music.woodCollision(); } catch (IOException ex) {
					 * }
					 */
					return;
				}
