...
public class AngryToadsDraw {
	...
	private void drawSling() {
		...
		if (!stagetodraw.isMouseJointNull()) {
			pen.drawLine((int) slingAnchor1.x, (int) slingAnchor1.y, (int) (slingpos.x + 16 * vpt.scale / 18f),
					(int) (slingpos.y + 12 * vpt.scale / 18f));
	...
	private void drawBackground() {
		...
		groundwidth = (int) ((((AngryToadsBodyInfo) ground.getUserData()).getAppearance().getWidth(null)
				+ (vpt.scale - 18f) * 5) / 1.6f);
		...
		for (int i = 0; i <= 6; i++) {
			pen.drawImage(((AngryToadsBodyInfo) ground.getUserData()).getAppearance(), (int) gpos.x, (int) gpos.y,
